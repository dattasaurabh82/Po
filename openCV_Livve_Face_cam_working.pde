...
// //all the libraries you need to call
/****/import java.awt.*;/************/
//
...
String selectedCom = Serial.list()[2];//com Port from the list(play around with your valus)
                                    //change a/c to your available list of COM Ports

void setup() {
  size(640, 480, P2D); // window size

  println(Serial.list()); // List COM-ports
  port = new Serial(this, selectedCom, 19200); // In my case it's COM9 and it's 2nd in the list
...
void draw() {
  ...
  //Texts on the applet
  fill(255, 255, 0);
  text("FaceTracker (@ saurabh datta)", 18, 22);
  ...
  text("x_pos: ", 18, 120);
  text("y_pos: ", 18, 135);
  text("Serial Port:", 18, 440);
  text(selectedCom, 90,440);
  text("Under APACCE License", 18, 460);
  ...
  for (int i = 0; i < faces.length; i++) {
    ...
    float servoVal = map(faceMid_X, 10, 630, 0, 180);
    int modVal = int(servoVal);
    midPosition.setValue(modVal);
